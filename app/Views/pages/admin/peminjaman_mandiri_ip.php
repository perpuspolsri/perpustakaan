...
<div class="w-full mx-auto text-[#505050]">
    <div class="w-full flex items-center justify-between gap-2">
        <div class="flex items-center gap-2">
            ...
            <h1 class="text-3xl font-semibold">Peminjaman Mandiri IP Address Permission</h1>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-3 mt-5" id="faq-area">
    </div>
    ...
    <div id="faq-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-2xl mx-4">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-[#505050]">Edit IP</h2>
...
<script>
    ...
    addMoreInput.addEventListener("click", function() {
        ...
        formArea.innerHTML += `<div class="flex flex-col gap-2">
                            <label for="body-faq" class="block text-sm font-medium text-t-gray">IP Ke-${inputCount}</label>
    ...
    faqModalButton.addEventListener("click", async function() {
        ...
        for (let i = 1; i <= inputCount; i++) {
            formArea.innerHTML += `<div class="flex flex-col gap-2">
                            <label for="body-faq" class="block text-sm font-medium text-t-gray">IP Ke-${i}</label>
    ...
    faqModalButtonSave.addEventListener("click", async function(e) {
        ...
        for (let i = 0; i < fileInputs.length; i++) {
            ...
            oldImages[i] = oldImages[i] + "&" + question;
        }
        ...
        try {
            ...
            const response = await Api.post('/api/admin/contents/peminjaman_mandiri_ip', {
                content: newContent,
                delete: toDelete
            });
            ...
            if (result.status === 'success') {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil!',
                    text: 'IP berhasil diperbarui ✅',
                    confirmButtonText: 'OK',
                    timer: 2000,
                    willClose: () => {
                        window.location.href = '/admin/peminjaman-mandiri-ip';
                    }
                });
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal!',
                    text: 'Gagal memperbarui IP ❌',
                    confirmButtonText: 'OK'
                });
            }
        } catch (err) {
            console.error(err);
            alert("Gagal memperbarui IP, coba lagi.");
        }
    });

    async function getfaqData() {
        try {
            const response = await Api.get('/api/public/contents/peminjaman_mandiri_ip');
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        for (let i = 1; i <= inputCount; i++) {
            faqArea.innerHTML += `<p class="border border-base-blue p-3 text-black rounded text-center font-bold tracking-widest" id="prev-question-${i}"></p>`
        ...
        while (i <= prevQuestion.length) {
            ...
            document.getElementById(prevQuestion[i]).innerHTML += arrContentsSliced[arrContentsSliced.length - 1]
            globalfaqName.push(arrContentsSliced[arrContentsSliced.length - 1])
            i++
        }
    });
